#ifndef PGC_HIST_MAT_H
#define PGC_HIST_MAT_H
// #define double float
...
namespace Pscf
{
   namespace Pspg
   {
      ...
      public:
         ...
         void evaluate(double elm, int nHist, int columnId);
      ...
      template <typename Data>
      void HistMat<Data>::evaluate(double elm, int nHist, int columnId)
      {
         if (nHist < maxHist_)
         { // if matrix is not filled, something tricky
            (*this)(maxHist_ - nHist, columnId + maxHist_ - nHist) = (double)elm;
         }
         else
         { // just fill the first row
      ...
      double HistMat<Data>::makeUmn(int i, int j, int nHist)
      {
         int offset;
         if (nHist < maxHist_)
         { // matrix is not filled, soemthing tricky
