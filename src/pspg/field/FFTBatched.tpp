#ifndef PGC_FFT_BATCHED_TPP
#define PGC_FFT_BATCHED_TPP
...
static __global__ void scaleComplexData(cudaComplex* data, cudaReal scale, int size) {
   ...
   */
   ...
   void FFTBatched<D>::forwardTransform(RDField<D>& rField, RDFieldDft<D>& kField)
   {
      ...
      scaleReal<<<NUMBER_OF_BLOCKS, THREADS_PER_BLOCK>>>(rField.cDField(), scale, rSize_ * 2);
   ...
   */
   ...
   (cudaReal* rField, cudaComplex* kField, int batchSize)
   {
      ...
      int NUMBER_OF_BLOCKS, THREADS_PER_BLOCK;
      ThreadGrid::setThreadsLogical(rSize_ * batchSize, NUMBER_OF_BLOCKS, THREADS_PER_BLOCK);
      // Copy rescaled input data prior to work array
      cudaReal scale = 1.0/cudaReal(rSize_);
      //scale for every batch
      scaleReal<<<NUMBER_OF_BLOCKS, THREADS_PER_BLOCK>>>(rField, scale, rSize_ * batchSize);
      ...
      scaleReal<<<NUMBER_OF_BLOCKS, THREADS_PER_BLOCK>>>(rField, cudaReal(rSize_), rSize_ * batchSize);
