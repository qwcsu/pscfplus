#ifndef PGC_FIELD_IO_TPP
#define PGC_FIELD_IO_TPP
...
namespace Pspg
{
   ...
   */
   ...
   // Create Mesh<D> with dimensions of DFT Fourier grid.
         ...
         if (starPtr->invertFlag == 0) {
            ...
            for (iw = starPtr->beginId; iw < starPtr->endId; ++iw) {
               ...
               if (!wavePtr->implicit) {
                  ...
                  // std::cout << rank << " : " << "dtf_out =" << dft_out[rank].x << " + i " << dft_out[rank].y << std::endl;
               }
            }
            ++is;
         ...
         if (starPtr->invertFlag == 1) {
            ...
            for (iw = starPtr->beginId; iw < starPtr->endId; ++iw) {
               ...
               if (!(wavePtr->implicit)) {
                  ...
                  dft_out[rank].x = coeff.real();
                  dft_out[rank].y = coeff.imag();
                  // std::cout << "dtf_out =" << dft_out[rank].x << " + i " << dft_out[rank].y << std::endl;
               }
            }
      ...
      // for (int i = 0; i < kSize; ++i)
      //    std::cout << "dtf_out =" << dft_out[0].x << " + i " << dft_out[0].y << std::endl;
     ...
     cudaMemcpy(dft_in, dft.cDField(),
      ...
      while (is < basis().nStar()) {
         ...
         if (starPtr->invertFlag == 0) {
            ...
            // UTIL_CHECK(abs(component.imag()) < 1e-6); // Did not satisfy 1.0E-8 accuracy due to float point accuracy
         ...
         if (starPtr->invertFlag == 1) {
            ...
            // Compute component value
            component = std::complex<double>(dft_in[rank].x, dft_in[rank].y);
            UTIL_CHECK(abs(wavePtr->coeff) > 1.0E-10);
            component /= wavePtr->coeff;
            component *= sqrt(2.0);
            components_out [is] = component.real();
